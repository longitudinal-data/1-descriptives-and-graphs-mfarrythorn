...
table_fun <- function(model){
  ...
  # nsim should be higher
  # oldNames = F for understandable names
  # new column called term that has row names
  
  ## Get ICC & R2 values ##
  ICC <- reghelper::ICC(model) # For model with no slope, unconditional cell means model
  ...
  # Selecting only the CIs for the fixed effects
 
  # variance of intercept, variance of slope, variance of correlation, variance of residuals 
  # If model doesn't have a random slope - remove the terms with 1 in them
  rand <- rand %>%
...
# tribble builds a table by rows (not columns)  
  mod_terms <- tribble(
    ~term, ~estimate, ~type,
    "ICC", ICC, "Model Terms",
  ...
  tab <- fixed %>%
    full_join(rand) %>%
    mutate(CI = sprintf("(%.2f, %.2f)", lower, upper)) %>% #round to two decimal points and puts in a comma
    select(-lower, -upper) %>%
    full_join(mod_terms) %>%
    mutate(estimate = sprintf("%.2f", estimate)) %>%
    select(type, everything()) # put type first and every other column in the order it was in
}
...
Why do people vary on the intercept? Now we're saying group might explain it. Random effects (U0j) will get smaller if group is explaining something. 

If group is also predicting slope. Why do some people have shallower slopes? Does group help explain that. U1j should get smaller. Will also change the γ10 - it's serving as an intercept - it's the average slope and we're adjusting up and down based on someone's intercept and residual.
